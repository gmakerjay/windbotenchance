...
namespace WindBot.Game
{
    public class GameBehavior
    {
        ...
        private void OnJoinGame(BinaryReader packet)
        {
            ...
            {
                Logger.WriteLine($"[Warning] Handshake mismatch: read {handshake}, expected {SERVER_HANDSHAKE}. Bypassing.");
            }
            ...
            if (Deck == null)
            {
                Logger.WriteErrorLine("Deck is null in OnJoinGame!");
                deck.Write(0);
                deck.Write(0);
            }
            else
        ...
        private void OnChangeSide(BinaryReader packet)
        {
            ...
            if (Deck == null)
            {
                Logger.WriteErrorLine("Deck is null in OnChangeSide!");
                deck.Write(0);
                deck.Write(0);
            }
            else
        ...
        private void OnReplay(BinaryReader packet)
        {
            ...
            if (!string.IsNullOrEmpty(Game.ReplayPath))
            {
                try
                {
                    string dir = Path.GetDirectoryName(Game.ReplayPath);
                    if (!string.IsNullOrEmpty(dir) && !Directory.Exists(dir))
                    {
                        Directory.CreateDirectory(dir);
                    }
                    File.WriteAllBytes(Game.ReplayPath, replay);
                    Logger.WriteLine("Replay saved successfully to: " + Game.ReplayPath);
                }
                catch (Exception ex)
                {
                    Logger.WriteErrorLine("Failed to save replay to: " + Game.ReplayPath + ", error: " + ex.Message);
                }
            }
            else
            {
        ...
        {
            if (Program.EventBus != null)
            {
                Program.EventBus.Publish(new DuelEvent
                {
                    DuelId = Game.DuelId ?? "duel",
                    Turn = _duel.Turn,
                    EventType = DuelEventType.DuelEnd,
                    CardName = "None",
                    BoardScore = _ai.Executor?.Scorer?.BoardAdvantageScore()
                });
            }
            Connection.Close();
        }
        ...
        private void OnErrorMsg(BinaryReader packet)
        {
            ...
            Logger.WriteErrorLine($"[OnErrorMsg] Received error message code: {msg}");
            // align
            packet.ReadByte();
            packet.ReadByte();
            packet.ReadByte();
            if (msg == 2) //ERRMSG_DECKERROR
            {
                ...
                Logger.WriteErrorLine($"[OnErrorMsg] DeckError Details: flag={flag}, code={code}");
        ...
        private void OnStart(BinaryReader packet)
        {
            ...
            Logger.StartDuelSession(_room.Names[0], _room.Names[1], Game.Username);
            HeuristicGuard.ResetSession();
            _ai.OnStart();
        }
        ...
        private void OnWin(BinaryReader packet)
        {
            ...
            // Layer 2+3: Log guard summary and dump board state on loss
            Logger.WriteTraceLine($"[GUARD-SUMMARY] {HeuristicGuard.GetSessionSummary()}");
            if (textResult == "Lose")
            {
                try { GameStateSnapshot.DumpToFile(_duel.Turn, _duel.Phase.ToString(), "Duel Lost", _duel.Fields[0], _duel.Fields[1]); } catch { }
            }

            Logger.EndDuelSession(textResult);
        }
        ...
        private void OnNewTurn(BinaryReader packet)
        {
            ...
            Logger.WriteLine("Turn " + _duel.Turn);
            _ai.OnNewTurn();
            ...
            if (Program.EventBus != null)
            {
                Program.EventBus.Publish(new DuelEvent
                {
                    DuelId = Game.DuelId ?? "duel",
                    Turn = _duel.Turn,
                    EventType = DuelEventType.PhaseChange,
                    Phase = "NewTurn",
                    CardName = "None",
                    BoardScore = _ai.Executor?.Scorer?.BoardAdvantageScore()
                });
        ...
        private void OnDamage(BinaryReader packet)
        {
            ...
            if (Program.EventBus != null)
            {
                Program.EventBus.Publish(new DuelEvent
                {
                    DuelId = Game.DuelId ?? "duel",
                    Turn = _duel.Turn,
                    EventType = DuelEventType.LpChange,
                    Player = player == 0 ? "A" : "B",
                    CardName = _duel.Fields[player].LifePoints.ToString(),
                    BoardScore = _ai.Executor?.Scorer?.BoardAdvantageScore()
                });
            }
        }
        ...
        private void OnMove(BinaryReader packet)
        {
            ...
            if (previous.location == (int)CardLocation.Deck && current.location == (int)CardLocation.Hand && current.controler == 1)
            {
                _ai.Executor?.Brain?.OnOpponentSearch();
            }
        ...
        private void OnChaining(BinaryReader packet)
        {
            ...
            if (Program.EventBus != null && card != null)
            {
                Program.EventBus.Publish(new DuelEvent
                {
                    DuelId = Game.DuelId ?? "duel",
                    Turn = _duel.Turn,
                    EventType = DuelEventType.Activate,
                    Player = cc == 0 ? "A" : "B",
        ...
        private void OnSelectSum(BinaryReader packet)
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    if (_duel == null)
                    {
                        Logger.WriteLine("OnSelectSum Error: _duel is null!");
                        continue;
                    }
                    ClientCard card = _duel.GetCard(info.controler, info.location, info.sequence, info.position);
                    if (card != null)
                    ...
                    {
                        Logger.WriteLine($"OnSelectSum Warning: GetCard returned null for cardId={cardId}, player={info.controler}, loc={info.location}, seq={info.sequence}, pos={info.position}");
                    }
            ...
            // --- Detailed Logging for OnSelectSum ---
            int tot_count = mandatoryCards.Count + selected.Count;
            Logger.WriteLine($"[OnSelectSum] mode={mode}, sumval={sumval}, min={min}, max={max}");
            Logger.WriteLine($"[OnSelectSum] mandatoryCount(packet)={mandatoryCount}, mandatoryCards.Count={mandatoryCards.Count}, selected.Count={selected.Count}, tot_count={tot_count}");
            for (int dbg = 0; dbg < mandatoryCards.Count; ++dbg)
            {
                Logger.WriteLine($"[OnSelectSum]   mandatory[{dbg}]: Id={mandatoryCards[dbg]?.Id}, SelectSeq={mandatoryCards[dbg]?.SelectSeq}, OpParam1={mandatoryCards[dbg]?.OpParam1}");
            }
            for (int dbg = 0; dbg < selected.Count; ++dbg)
            {
                Logger.WriteLine($"[OnSelectSum]   selected[{dbg}]: Id={selected[dbg]?.Id}, SelectSeq={selected[dbg]?.SelectSeq}, OpParam1={selected[dbg]?.OpParam1}");
            }

            // Build response matching EXACTLY the ProjectIgnis/windbot original format:
            // Bytes [0-3]: int32 = 2 (selection_value)
            // Bytes [4-7]: int32 = tot_count (mandatory + selected)
            // Bytes [8..]: 0 per mandatory card, then SelectSeq per selected card (1 byte each)
            byte[] result = new byte[mandatoryCards.Count + selected.Count + 16];
            int selection_value = 2;
            result[0] = (byte)(selection_value & 0xff);
            result[1] = (byte)((selection_value >> 8) & 0xff);
            result[2] = (byte)((selection_value >> 16) & 0xff);
            result[3] = (byte)((selection_value >> 24) & 0xff);

            uint tot_count_u = (uint)(mandatoryCards.Count + selected.Count);
            result[4] = (byte)(tot_count_u & 0xff);
            result[5] = (byte)((tot_count_u >> 8) & 0xff);
            result[6] = (byte)((tot_count_u >> 16) & 0xff);
            result[7] = (byte)((tot_count_u >> 24) & 0xff);

            int index = 8;
            // Mandatory cards: write 0 for each (they are pre-selected by the engine)
            for (int k = 0; k < mandatoryCards.Count; ++k)
            {
                result[index++] = 0;
                Logger.WriteLine($"[OnSelectSum]   -> mandatory[{k}] = 0");
            }
            // Selected cards: write SelectSeq for each
            for (int l = 0; l < selected.Count; ++l)
            {
                result[index++] = (byte)selected[l].SelectSeq;
                Logger.WriteLine($"[OnSelectSum]   -> selected[{l}] = SelectSeq={selected[l].SelectSeq} (Id={selected[l]?.Id})");
            }

            Logger.WriteLine($"[OnSelectSum] Response: selVal=2, totCount={tot_count_u}, bytes={index}, payload=[{string.Join(",", new ArraySegment<byte>(result, 0, index))}]");

            BinaryWriter reply = GamePacketFactory.Create(CtosMessage.Response);
            reply.Write(result, 0, index);
            Connection.Send(reply);
        }
        ...
        private void OnSelectYesNo(BinaryReader packet)
        {
            ...
            if (desc == 30)
                ...
                reply = _ai.OnSelectBattleDirectAttack(_duel.Fields[0].BattlingMonster, true) ? 1 : 0;
            else if (desc == 1989)
                reply = 1;
            else
        ...
        {
            int selected = _ai.OnAnnounceCard();
            Logger.WriteLine($"[OnAnnounceCard] Announcing card ID: {selected}");
            Connection.Send(CtosMessage.Response, selected);
        }
        ...
        private void OnAnnounceRace(BinaryReader packet)
        {
            ...
            long remaining = packet.BaseStream.Length - packet.BaseStream.Position;
            ulong available = 0;
            bool isLegacy = remaining <= 4;
            
            if (isLegacy)
            {
                available = (ulong)packet.ReadInt32();
                Logger.WriteLine($"[OnAnnounceRace] Received (Legacy 32-bit): count={count}, available=0x{available:X8}");
            }
            else
            {
                available = packet.ReadUInt64();
                Logger.WriteLine($"[OnAnnounceRace] Received (Modern 64-bit): count={count}, available=0x{available:X16}");
            }
            
            uint filter = 0x1;
            for (int i = 0; i < 32; ++i)
            {
                if (((uint)available & filter) != 0)
                    races.Add((CardRace)filter);
                filter <<= 1;
            }
            Logger.WriteLine($"[OnAnnounceRace] Available races parsed ({races.Count}): {string.Join(", ", races)}");
            races = _ai.OnAnnounceRace(count, races);
            Logger.WriteLine($"[OnAnnounceRace] AI selected races ({races?.Count}): {(races != null ? string.Join(", ", races) : "null")}");
            
            int reply = 0;
            for (int i = 0; i < count; ++i)
            {
                if (races != null && i < races.Count)
                {
                    reply += (int)races[i];
                }
                else
                {
                    Logger.WriteLine($"[OnAnnounceRace] Warning: requested count={count} but only {races?.Count ?? 0} races were available/selected.");
                }
            }
            
            Logger.WriteLine($"[OnAnnounceRace] Sending Response (isLegacy={isLegacy}) reply=0x{reply:X8} ({reply})");
            BinaryWriter replyPacket = GamePacketFactory.Create(CtosMessage.Response);
            if (isLegacy)
            {
                replyPacket.Write(reply);
            }
            else
            {
                replyPacket.Write((long)reply);
            }
            ...
            foreach (ClientCard card in _duel.SummoningCards)
            {
                ...
                if (Program.EventBus != null && card != null)
                {
                    Program.EventBus.Publish(new DuelEvent
                    {
                        DuelId = Game.DuelId ?? "duel",
                        Turn = _duel.Turn,
                        EventType = DuelEventType.Summon,
                        Player = card.Controller == 0 ? "A" : "B",
                        CardName = card.Name ?? "Unknown",
                        BoardScore = _ai.Executor?.Scorer?.BoardAdvantageScore()
                    });
                }
                ...
                if (card != null)
                {
                    _ai.Executor?.Brain?.OnMonsterSummoned(card, false);
                }
            }
            _duel.SummoningCards.Clear();
        }
            ...
            foreach (ClientCard card in _duel.SummoningCards)
            {
                ...
                if (Program.EventBus != null && card != null)
                {
                    Program.EventBus.Publish(new DuelEvent
                    {
                        DuelId = Game.DuelId ?? "duel",
                        Turn = _duel.Turn,
                        EventType = DuelEventType.SpecialSummon,
                        Player = card.Controller == 0 ? "A" : "B",
                        CardName = card.Name ?? "Unknown",
                        BoardScore = _ai.Executor?.Scorer?.BoardAdvantageScore()
                    });
                ...
                if (card != null)
                {
                    _ai.Executor?.Brain?.OnMonsterSummoned(card, true);
                }
            }
            _duel.SummoningCards.Clear();
        }
