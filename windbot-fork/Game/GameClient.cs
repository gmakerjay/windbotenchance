...
namespace WindBot.Game
{
    public class GameClient
    {
        ...
        private void OnDisconnected(System.Exception ex)
        {
            if (ex != null)
            {
                Log("Connection lost / failed: " + ex.Message + "\n" + ex.StackTrace, 2);
                Logger.EndDuelSession("Error / Disconnected");
            }
            else
            {
                Log("Connection closed by remote host.", 0);
                Logger.EndDuelSession("Closed / Disconnected");
            }
        }
