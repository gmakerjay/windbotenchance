...
namespace WindBot.Game.AI.Decks
{
    ...
    class ABCExecutor : DefaultExecutor
    {
        ...
        private bool ABCBanish()
        {
            ...
            else AI.SelectCard(CardLocation.Hand);
        ...
        private bool KnightmarePhoenixEffect()
        {
            ...
            else if (Bot.HasInGraveyard(CardId.BBusterDrake) && Bot.HasInGraveyard(CardId.CCrushWyvern) && Bot.HasInHand(CardId.AAssaultCore))
                AI.SelectCard(CardId.AAssaultCore);
            else if (Bot.HasInGraveyard(CardId.CCrushWyvern) && Bot.HasInGraveyard(CardId.AAssaultCore) && Bot.HasInHand(CardId.BBusterDrake))
                AI.SelectCard(CardId.BBusterDrake);
            else if (Bot.HasInHand(discards))
        ...
        private bool KnightmareCerberusEffect()
        {
            ...
            if (target == null)
                return false;
            if (!target.IsSpecialSummoned && target.Sequence >= 5)
                return false;
            if (Bot.HasInGraveyard(CardId.AAssaultCore) && Bot.HasInGraveyard(CardId.BBusterDrake) && Bot.HasInHand(CardId.CCrushWyvern))
                AI.SelectCard(CardId.CCrushWyvern);
            else if (Bot.HasInGraveyard(CardId.BBusterDrake) && Bot.HasInGraveyard(CardId.CCrushWyvern) && Bot.HasInHand(CardId.AAssaultCore))
