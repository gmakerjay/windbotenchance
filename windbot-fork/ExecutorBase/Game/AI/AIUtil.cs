...
namespace WindBot.Game.AI
{
    public class AIUtil
    {
        ...
        /// <summary>
        /// Check if the opponent controls any active, non-disabled cards that can negate monster/spell/trap effects or activations.
        /// </summary>
        public bool OpponentHasNegation()
        {
            // Negation monster IDs
            int[] negaterMonsterIds = {
                10443957, // Cyber Dragon Infinity
                4280258,  // Apollousa, Bow of the Goddess
                84815190, // Baronne de Fleur
                21522601, // Witchcrafter Madame Verre (Corrected)
                84523092, // Witchcrafter Haine (Disruption)
            ...
            foreach (ClientCard card in Enemy.MonsterZone)
            {
                ...
                {
                    if (negaterMonsterIds.Contains(card.Id) || card.IsMonsterShouldBeDisabledBeforeItUseEffect())
                        return true;
                }
            }
            ...
            foreach (ClientCard card in Enemy.SpellZone)
            {
                if (card != null && card.IsFaceup() && !card.IsDisabled())
                {
                    if (card.Id == 27541563 || // Altergeist Protocol (Corrected)
                        card.Id == 87769556 || // Magician's Right Hand
                        card.Id == 13758665)   // Magician's Left Hand
                        return true;
                }
            }
        ...
        private static readonly Random _shuffleRand = new Random();
        ...
        public void ShuffleListInPlace<T>(IList<T> list)
        {
            ...
            {
                n--;
                int index = _shuffleRand.Next(n + 1);
                T value = list[index];
                list[index] = list[n];
                list[n] = value;
            }
