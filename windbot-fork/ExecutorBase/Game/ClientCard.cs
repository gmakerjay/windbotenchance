...
namespace WindBot.Game
{
    public class ClientCard
    {
        ...
        public int Rank
        {
            get
            {
                if (_rank != 0) return _rank;
                if (HasType(CardType.Xyz) && Data != null) return Data.Level;
                return 0;
            }
        ...
        public int Type
        {
            get { return _type != 0 ? _type : (Data?.Type ?? 0); }
        ...
        public int LinkCount
        {
            get
            {
                if (_linkCount != 0) return _linkCount;
                if (HasType(CardType.Link) && Data != null) return Data.Level;
                return 0;
            }
            private set { _linkCount = value; }
        }
        ...
        public long Update(BinaryReader packet, Duel duel)
        {
            ...
            while (true)
            {
                ...
                switch (flag)
                {
                    ...
                    default:
                        {
                            int skip = size - sizeof(uint);
                            if (skip > 0)
                            {
                                packet.BaseStream.Position += skip;
                            }
                            break;
                        }
                }
