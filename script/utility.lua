...
-- Fallbacks for Duel.GetReasonEffect and Duel.GetReasonPlayer
local ok_re, getReasonEffect = pcall(function() return Duel.GetReasonEffect end)
if not ok_re or not getReasonEffect then
	Duel.GetReasonEffect = function() return nil end
else
	local ok_call = pcall(Duel.GetReasonEffect)
	if not ok_call then
		Duel.GetReasonEffect = function() return nil end
	end
end

local ok_rp, getReasonPlayer = pcall(function() return Duel.GetReasonPlayer end)
if not ok_rp or not getReasonPlayer then
	Duel.GetReasonPlayer = function() return Duel.GetTurnPlayer() end
else
	local ok_call = pcall(Duel.GetReasonPlayer)
	if not ok_call then
		Duel.GetReasonPlayer = function() return Duel.GetTurnPlayer() end
	end
end
...
function Auxiliary.DelayedOperation(card_or_group,phase,flag,e,tp,oper,cond,reset,reset_count,hint,effect_desc)
	local g=(type(card_or_group)=="Group" and card_or_group:Clone() or Group.FromCards(card_or_group))
	...
	local function get_affected_group(e)
		...
		local ok,res=pcall(function() return g:Filter(agfilter,nil,e:GetLabel()) end)
