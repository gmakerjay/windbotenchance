...
function Duel.GetTargetGroup(filter,player,loc1,loc2,exclusion,...)
	...
	local g=Duel.GetMatchingGroup(filter,player,loc1,loc2,exclusion,...)
	if re then
		return g:Match(Card.IsCanBeEffectTarget,nil,re)
	else
		return g:Filter(function(c) return not c:IsHasEffect(EFFECT_CANNOT_BE_EFFECT_TARGET) end,nil)
	end
end
